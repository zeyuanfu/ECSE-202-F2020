 ...
 * with potential collisions with fixed obstacles in the scene.
 *
 */
...
public class ppBall extends Thread {
	...
	public ppBall(double Xinit, double Yinit, double Vo, double theta, Color color, double loss, ppTable table, ppPaddle myPaddle) {
	...
	public void run() {
		...
		if (Vx<0 && (Xo+X)<=XLWALL+bSize) {
			...
			time=0; 													// Reset current interval time
			Xo=XLWALL+bSize; 											// Update X and Y offsets
			Yo+=Y;
			X=0; 														// Reset X to zero for start of next interval
			Y=0;
		}
